...
class RemovePoints(commands.Cog):
    ...
    @app_commands.command(name="remove-points", description="Removes points from an existing user.")
    async def remove_points(self, interaction: discord.Interaction, username: str, amount: int):
        ...
        try:
            ...
            if user_name_lower in usernames:
                ...
                embed = discord.Embed(color=discord.Color.green(), title="Success", description=f'{amount} points have been removed from {username}. New total: {new_points} points.')
                await interaction.followup.send(embed=embed, ephemeral=True)
